...
from database import get_db, Product, ProductRecipe, Category, write_log, format_rp, parse_rp
...
try:
        ...
        if not products:
            ...
            for i in range(0, len(products), 2):
                ...
                for j, col in enumerate(cols):
                    ...
                    p = products[i + j]
                    with col:
                        status_badge = {"approved": "🟢", "pending": "🟡", "rejected": "🔴"}.get(p.approval_status or "approved", "⚪")
                        ...
                        with st.container(border=True):
                            ...
                            with c_img:
                                if p.image_path and os.path.isfile(p.image_path):
                                    ...
                                    grad = "linear-gradient(135deg,#3f1d2e,#7f1d1d)" if is_dark() else "linear-gradient(135deg,#ffe4e6,#fecdd3)"
                                    st.markdown(f'<div style="background:{grad};border-radius:12px;height:140px;display:flex;align-items:center;justify-content:center;font-size:3rem;">🍓</div>', unsafe_allow_html=True)
                            with c_info:
                                st.markdown(header)
                                st.caption(f"{(p.product_type or 'pure').upper()} · {active_txt} · {p.approval_status or 'approved'}")
                                if p.created_by:
                                    st.caption(f"Oleh: {p.created_by}")

                            with st.expander("Detail & resep", expanded=False):
                                if p.description:
                                ...
                                if is_owner and (p.approval_status or "approved") == "approved":
                                    ...
                                    price_txt = st.text_input("Harga (Rp)", key=pk)
                                    parsed = parse_rp(price_txt)
                                    st.caption(f"Preview: **{format_rp(parsed)}**")
                                    img_up = st.file_uploader("Ganti gambar", type=["jpg", "jpeg", "png", "webp"], key=f"img_{p.id}")
                                    ...
                                    if st.button("Simpan perubahan", key=f"upd_{p.id}", type="primary"):
                                        ...
                                        if img_up is not None:
                                            ...
                                            p.image_path = path
                                        write_log(db, user, "product.update", f"Update produk {p.name} harga {format_rp(parsed)}",
    ...
    with tab_add:
        if not cats:
            ...
            price_raw = st.text_input("Harga dasar (Rp/kg) *", value="35000", key="new_price_txt")
            ...
            st.markdown("**Resep (rasio 0–1)**")
            ratios = {}
            rcols = st.columns(len(cats))
            for i, c in enumerate(cats):
                with rcols[i]:
                    default = 1.0 if (ptype == "pure" and i == 0) else 0.0
                    ratios[c.id] = st.number_input(c.name, min_value=0.0, max_value=1.0, value=default, step=0.05, format="%.2f", key=f"ratio_{c.id}")
            ...
            if st.button(btn_label, type="primary", use_container_width=True):
                ...
                if not name.strip():
                    ...
                    approved = is_owner
                    p = Product(name=name.strip(), product_type=ptype, base_price=float(price),
                        description=desc or None, approval_status="approved" if approved else "pending",
                        is_active=True if approved else False, created_by=user["name"])
                    db.add(p)
                    db.flush()
                    ...
                    if img is not None:
                        ...
                        p.image_path = path
                    write_log(db, user, "product.create" if approved else "product.request",
                        f"{'Buat' if approved else 'Ajukan'} produk {p.name} ({format_rp(price)})",
                        entity_type="product", entity_id=p.id)
                    db.commit()
                    flash_success(f"✅ Produk '{name}' aktif · {format_rp(price)}" if approved else f"📤 Produk '{name}' diajukan.")
                    st.rerun()

    if tab_appr is not None:
        with tab_appr:
            pending = db.query(Product).filter(Product.approval_status == "pending").order_by(Product.created_at.asc()).all()
            if not pending:
                ...
                for p in pending:
                    with st.container(border=True):
                        ...
                        recipes = p.recipes or []
                        if recipes:
                            st.write("Resep: " + ", ".join(f"{r.category.name if r.category else r.category_id} {r.ratio*100:g}%" for r in recipes))
                        a1, a2 = st.columns(2)
                        with a1:
                            if st.button("Approve & Aktifkan", key=f"ap_prod_{p.id}", type="primary"):
                                p.approval_status = "approved"
                                p.is_active = True
                                write_log(db, user, "product.approve", f"Approve produk {p.name}", entity_type="product", entity_id=p.id)
                                db.commit()
                                flash_success(f"Produk '{p.name}' disetujui.")
                                st.rerun()
                        with a2:
                            if st.button("Tolak", key=f"rj_prod_{p.id}"):
                                p.approval_status = "rejected"
                                p.is_active = False
                                write_log(db, user, "product.reject", f"Tolak produk {p.name}", entity_type="product", entity_id=p.id)
                                db.commit()
